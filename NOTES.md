...
없음)라 잘렸지만, **남은 9.16~10.81 s 는 마지막 반복의 끝 — 일어서기의 마무리(9.2~9.6 s) 와 다시 앉는 동작
(10.0~10.6 s, 골반 0.94 → 0.58 m, 의자에 닿기 직전까지)** 이라 의자 힘 없이 쓸 수 있다(잔차 1 %).
단, 운동학적으로는 스쿼트 내려가기와 거의 같다 — STS 고유의 어려운 순간(몸통을 앞으로 숙여
엉덩이를 떼는 seat-off, 고관절 토크 피크)은 의자 힘이 필요한 구간이라 잘려 있다. 케이블 근육으로 재현: 오른다리 토크 오차 최악
