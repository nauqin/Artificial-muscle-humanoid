...
STS ID 가 의자 힘을 포함한 값인지 — **확인했다 (9/15): 포함 안 됐다.**

- 논문(Uhlrich 2023, Methods): 힘판은 "three in-ground force plates (Bertec)". STS 절차는
  "performing five sit-to-stands naturally and then with increased trunk flexion (forward lean when
  rising)", "rising from a 40 cm chair", 분석은 "averaged over the rising phase" (3회 반복). 의자를
  힘판 위에 뒀다는 말은 없다.
- 데이터: `ForceData/STS1_forces.mot` 에 `3_ground_force_*` 열이 있지만 **시행 내내 0**. 앉아 있는
  동안(골반 높이 0.58 m) 발 힘판 합은 체중의 19~36 %, 일어서면 85~150 %. 즉 의자가 받친 65~80 %
  는 측정에 없다. squats1 의 힘판 3 도 0(안 씀).
- 뜻: 논문 팀 STS ID 는 앉아 있는 구간에서 틀리다(의자 힘 없이 계산). 논문은 rising phase 만 써서
  무관하지만, 우리 방향 설계는 STS 시행 **전체** 18,606 프레임을 넣었다. 발 힘판 합 ≥ 0.85 BW 인
  프레임만 남기는 안도 있었지만, 그 구간은 스쿼트와 운동학이 같아 새 정보가 없다 → **STS 를 설계에서
  뺐다 (9/15, 사용자 결정: 걷기 + 스쿼트).** v1 결과는 `report/archive_v1/` 로 옮겼다.
- AddB 의 STS1_segment_0(9.16~10.81 s)이 남긴 이유도 같다: 발 힘판 합이 체중 근처인 구간만 남긴 것.
...
### 설계 v2(걷기 + 스쿼트)와 경로 2단계 — 근육별로 방향만 맞춘 경로는 깊은 스쿼트에서 모자랐다 (2026-09-15)

**설계 v2**: `design_directions.py` 의 과제를 `("squats1",)` 만으로(STS 제거) 다시 돌렸다. 27,400 프레임,
총용량 **394 Nm**(v1 과 같음; 축 정렬 522, NMF 521), 걷기 400 · 스쿼트 262. 용량
[57.4, 25.2, 39.2, 71.8, 46.2, 24.7, 84.1, 45.8]. v1 과 방향 코사인 [0.94, 0.27, 0.92, 0.89, 0.94, 0.89, 0.34, 0.42]
— M1·M6·M7 이 바뀌었다. 토크형 7명 검증(`_design2_scaled_rw100`): 10/12 걸음 보조 ≤ 0.8 %, 5·10번은
v1 과 같은 좌표·구간의 Moco 튐 노드(고관절 내전 125 %, 고관절 굴곡 196 %) — 설계 문제 아님.

**경로 1단계를 v2 방향으로 다시 만들자 스쿼트가 안 됐다**: 보조 최악 29 %, 케이블 토크 vs ID 17 %
(v1 경로는 5.6 %). 원인 추적(정적 검사, `report/static_check.py` = 케이블의 실제 모멘트암
r_i(q_t) 로 프레임마다 0~1 제약 최소제곱):
- 토크형(방향 고정) 잔차: 스쿼트·걷기 모두 **0 %** → 방향 설계는 문제없다.
- v2 1단계 케이블: 고관절 107°·무릎 99° 프레임에서 고관절 굴곡 **16.7 %**, 발목 16.5 % 부족. v1 케이블은 1 %.
- 1단계 비용을 "평균 + 최악 자세" 방향오차로 바꿔도 **27.9 %** 로 더 나빠졌다. 근육 하나의 방향을 맞추는
  것과 8개를 합쳐 토크를 내는 것은 다른 문제다. v1 이 됐던 건 운이었다.

**2단계(`path_design.refine`)**: 8개 부착점 + 힘 배율 f_i ∈ [1, 3] 을 Powell 로 한꺼번에 조정. 힘 =
f_i × 1.1 × 용량 ÷ 최소 유효 모멘트암(설계 용량은 항상 보장). 비용 = 표본 121 프레임(걷기 60 + 스쿼트 60
+ 무릎 최대 프레임, ID 토크와 짝)에서의 잔차(최악 + 0.3 평균, ID 피크 대비) + 3 × 방향오차 합 + 0.02 ×
평균(f − 1) + 스트로크·힘 상한·최소 모멘트암 벌점. 프레임별 box-lsq 는 FISTA 로 벡터화(4 ms/평가, 정확해와
일치 확인). 20,000 평가 90 초. 결과: 잔차 9.7 → **4.1 %**, 각도차 0.6~7.7°, 힘 590~3,460 N(배율은 M1 만
1.4), 스트로크 ≤ 0.19. 정적 검사 스쿼트·걷기 전 프레임 **0 %**.

첫 시도의 실패: 힘을 자유 변수(200~5,000 N)로 두고 벌점 0.1 × 평균(F/5,000) 을 주자 Powell 이 힘을 줄이는
쪽으로 가며 케이블 6개를 설계 방향과 50~140° 다른 곳에 붙였다(잔차 16 %). 비용은 내려갔지만 설계 방향이
사라진다. → 힘을 용량에 묶고 방향오차 항을 남겼다. 벌점이 거의 없던 그 전 시도는 힘을 5,000 N 으로
올려 0 % 를 만들었다. 비용에 적은 대로만 움직인다.

**Moco 검증 (v3 경로, `results_paths.csv` 의 `_v3`)** — 오른다리 케이블 토크 vs ID 최악 / 보조 최악:

| subject3 | walking1 | walking2 | walking3 | TS2 | TS3 | TS4 | squats1 2.6~4.4 | walking1 0.03~1.27 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 토크 vs ID | 5.5 % | 6.3 | 2.8 | 19.1 (튐 노드 9개 제외; 포함 104) | 3.8 | 4.3 | 9.5 | 12.5 (0.25~1.12 창, 튐 노드 제외, 무릎) |
| 보조 | 4.8 % | 4.5 | 4.9 | 3.3 | 3.5 | 4.0 | 9.2 | 5.6 |

v1 경로(6.7 / 5.3 / 7.9 / 23.2 / 7.9 / 10.1, 스쿼트 5.6 미수렴)보다 걷기는 전부 좋아졌고 스쿼트는 수렴한 채
9.5 %. 튐 노드 제외 계산: 노드마다 "케이블 토크 + 보조 = Moco 내부 토크" 가 우리 ID 와 5 Nm 넘게 다르면
제외(`report/artifact_metric.py`; v1 TS2 로 돌리면 예전 값 23.2 가 그대로 나와 방법이 같음을 확인).
걷기 전체 구간의 12.5 % 는 무릎(ID 피크 27 Nm 에 3.4 Nm)이고 튐 노드 옆이다.

파일 정리: 영상 `video_paths_subject3_{walking1,squats1}.mp4`(보조 패널 추가, v3 경로), GUI
`gui_muscles_model.osim` + `gui_paths_subject3_{walking1_full_v3,squats1_v3}.mot` — **GUI 모션은 모델과 짝**이라
v1 경로의 gui .mot 는 지웠다. STS 영상은 `archive_v1/` 로. `_v2` 태그의 Moco 해(실패한 1단계 경로)는
`results_paths.csv` 에 기록만 남아 있다.

남은 것: 뼈 간섭(직선 케이블이 깊은 스쿼트에서 뼈를 가로지름, `fig_path_layout.png`), 6/8 이 4바디 경로.
→ 경로 3차에서 관절 중심 최소거리 + wrap + 건너는 관절 수 벌점.
